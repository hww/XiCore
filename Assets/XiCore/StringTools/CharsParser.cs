...
namespace XiCore.StringTools
{
    ...
    public static class CharsParser
    {
        ...
        /// </exception>
        public static double Parse(char[] input, int start, int ends, out int end)
        {
            if (input == null)
            {
            ...
            {
                throw new ArgumentOutOfRangeException("ends", "Value must be less then input.Length");
            }
            
            int endOfWhitespace = SkipWhiteSpace(input, start, ends);

            long significand;
            int significandsExponent, sign;
            int startOfDigits = ParseSign(input, endOfWhitespace, ends, out sign);
            int index = SkipLeadingZeros(input, startOfDigits, ends);
            index = ParseSignificand(input, index, ends, out significand, out significandsExponent);

            // Have we parsed a number?
            if ((index - startOfDigits) > 0)
            {
                int exponent;
                end = ParseExponent(input, index, ends, out exponent);
                return MakeDouble(significand * sign, exponent - significandsExponent);
            }

            // Not a number, is it a constant?
            double value;
            end = ParseNamedConstant(input, endOfWhitespace, ends, out value);
        ...
        private static int ParseExponent(char[] str, int startIndex, int ends, out int exponent)
        {
            exponent = 0;
            ...
            if (index < ends)
            {
                ...
                {
                    int sign;
                    index = ParseSign(str, index + 1, ends, out sign); // Add one to the index to skip the 'e'

                    int digitStart = index; // Keep a track of if we parse any digits
                    index = SkipLeadingZeros(str, index, ends);

                    long value = 0;
                    index = ParseNumber(3, str, index, ends, ref value);
        ...
        private static int ParseNamedConstant(char[] str, int startIndex, int ends, out double value)
        {
            // StartsWith only converts str to uppercase - important we pass the constant in uppercase
            if (StartsWith(str, startIndex, ends, "NAN"))
            {
                value = double.NaN;
                return startIndex + 3;
            }
            ...
            int sign;
            int index = ParseSign(str, startIndex, ends,out sign); 
            if (StartsWith(str, index, ends, "INFINITY"))
            {
                value = sign * double.PositiveInfinity;
                return index + 8;
            }
            if (StartsWith(str, index, ends,"INF"))
            {
                value = sign * double.PositiveInfinity;
                return index + 3;
            }
        ...
        private static int ParseNumber(int maxDigits, char[] str, int index, int ends, ref long value)
        {
            for (; index < ends; index++)
            {
                char c = str[index];
                if ((c < '0') || (c > '9'))
                {
        ...
        private static int ParseSign(char[] str, int index, int ends, out int value)
        {
        ...
        private static int ParseSignificand(char[] str, int startIndex, int ends, out long significand, out int exponent)
        {
            exponent = 0;
            significand = 0;
            int index = ParseNumber(18, str, startIndex, ends, ref significand);
            int digits = index - startIndex;

            // Is there a decimal part as well?
            if ((index < ends) && (str[index] == '.'))
            {
                ...
                index = ParseNumber(18 - digits, str, index, ends, ref significand);
        ...
        private static int SkipLeadingZeros(char[] str, int index, int ends)
        {
            for (; index < ends; index++)
            {
                if (str[index] != '0')
        ...
        private static int SkipWhiteSpace(char[] str, int index, int ends)
        {
            for (; index < ends; index++)
            {
                char c = str[index];
        ...
        private static bool StartsWith(char[] str, int startIndex, int ends, string value)
        {
            for (int i = 0; i < value.Length; i++)
            {
                if ((startIndex + i) >= ends)
                {
                    return false;
                }
